import re
import json
import markdown  # Pode ser substituído por mistune se preferir
...
def renderizar_estrutura(texto_bruto):
    ...
    for match in BLOCO_PATTERN.finditer(texto_bruto):
        ...
        if prosa_antes:
            ...
            resultado.append({
                "tipo": "markdown",
                "html": markdown.markdown(prosa_antes, extensions=['extra'])
            })
    ...
    if prosa_depois:
        resultado.append({
            "tipo": "markdown",
            "html": markdown.markdown(prosa_depois, extensions=['extra'])
        })
